...
conn = psql.connect("dbname='grad' user='mongo' host='localhost' password='mongo'")
print(conn)
...
#	print(user["points"])
	...
	for finish in user["finished"]:
		curPSQL.execute("""INSERT INTO finished VALUES ("""+str(user['_id'])+""",DEFAULT,"""+str(finish)+""");""")

	curPSQL.execute("""INSERT INTO favorites VALUES (DEFAULT,"""+str(user['_id'])+""",'"""+str(user['favorites']['artist'])+"""',
							'"""+str(user['favorites']['food'])+"""');""")

	for badge in user["badges"]:
		curPSQL.execute("""INSERT INTO badges VALUES (DEFAULT,"""+str(user['_id'])+""",'"""+str(badge)+"""');""")
